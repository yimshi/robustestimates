...
#OP
filedir=paste0("filepath/OP",beta1,"/")
...
for (III in 1:iters){
  ...
  datam[,1]<- rnbinom(n,size=exp(eta),p=0.5)
  datam[,2]<- x
...
saveWorkbook(wb, file = "OPtypeIerror.xlsx",overwrite=T)
####################################################################################################
...
# Save the workbook to a file
saveWorkbook(wb, file = "OPpower.xlsx",overwrite=T)
