...
#Poisson
filedir=paste0("filepath/Poisson",beta1,"/")
...
for (III in 1:iters){
  ...
  datam[,1]<- round(rpois(n,lambda=exp(eta)))
  datam[,2]<- x
...
saveWorkbook(wb, file = "PoissontypeIerror.xlsx",overwrite=T)
####################################################################################################
...
# Save the workbook to a file
saveWorkbook(wb, file = "Poissonpower.xlsx",overwrite=T)
