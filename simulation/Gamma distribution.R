...
#gamma 
filedir=paste0("filepath/Gamma",beta1,"/")
...
for (III in 1:iters){
  ...
  eta<-beta[1]+ beta[2]* x
  mu=exp(eta)
  datam[,1]<- rgamma(n,shape=mu^1.5, scale=mu^(-0.5))
  datam[,2]<- x
  
  filename1=paste(filedir, III, ".csv", sep="")	
...
saveWorkbook(wb, file = "GammatypeIerror.xlsx",overwrite=T)
####################################################################################################
...
# Save the workbook to a file
saveWorkbook(wb, file = "Gammapower.xlsx",overwrite=T)
