...
#Pareto
filedir=paste0("filepath/Pareto",beta1,"/")
...
for (III in 1:iters){
  ...
  eta<-beta[1]+ beta[2]* x
  #mean=exp(eta)
  datam[,1]<- rpareto1(n,shape=al,min=(al-1)*exp(eta)/al)
  datam[,2]<- x
...
saveWorkbook(wb, file = "ParetotypeIerror.xlsx",overwrite=T)
####################################################################################################
...
# Save the workbook to a file
saveWorkbook(wb, file = "Paretopower.xlsx",overwrite=T)
